...
const f = async () => {
    ...
    console.log(user);
    const allwords = await UserDictionary.getAllAggregatedUserWords(result, sign.token, '1', '1', '10');
    console.log(allwords);
    const getagwords = await UserDictionary.getAggregateUserWord({
        userId: result,
        wordId: '5e9f5ee35eb9e72bc21af701',
        token: sign.token,
    });
    console.log(getagwords);
